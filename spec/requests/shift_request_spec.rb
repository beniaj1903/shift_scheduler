...
RSpec.describe "Shifts", type: :request do
    describe "create" do
        ...
        it "creates a service week shifts" do
            mock = double("Shift", shifts: [
                { service_id: 1, week_id: 1, day: 1, start_time: Time.now.beginning_of_hour, end_time: (Time.now.beginning_of_hour + 1) },
                { service_id: 1, week_id: 1, day: 2, start_time: Time.now.beginning_of_hour, end_time: (Time.now.beginning_of_hour + 1) },
                { service_id: 1, week_id: 1, day: 3, start_time: Time.now.beginning_of_hour, end_time: (Time.now.beginning_of_hour + 1) },
                ] )
    ...
    describe "simple distribute verifications" do
        ...
        it "verificates all sended shifts are distributed" do 
            headers = { "ACCEPT" => "application/json" }
            post "/api/v1/shifts/distribute", :params => { :shift_ids => shifts.map(&:id), :employee_ids => employees.map(&:id) }, :headers => headers
            expect(response.content_type).to eq("application/json; charset=utf-8")
            expect(response).to have_http_status(:ok)
    ...
    describe "complex distribute verifications" do
        ...
        it "verificates diferences between employees shifts distribution are <= 1, when the availabilities are similar" do 
            headers = { "ACCEPT" => "application/json" }
            post "/api/v1/shifts/distribute", :params => { :shift_ids => shifts[:large_shift_array].map(&:id), :employee_ids => shifts[:employees].map(&:id) }, :headers => headers

            # maps in a hash the shift count for each employee
            employees_count_hash = shifts[:employees].inject({}) do |obj, employee| 
                obj[employee.id.to_s] = 0
                obj
            end
            JSON.parse(response.body)['shifts'].each do |shift|
                employee_id = shift.values[0]['employee_id']
                if !employee_id.blank?
                    employees_count_hash[employee_id.to_s] += 1
                end
            end
            p "employees_count_hash"
            p employees_count_hash
            p "employees_count_hash"
