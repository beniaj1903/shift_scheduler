...
RSpec.describe Shift, type: :model do
  describe 'attributes' do
    ...
    let!(:shifts) do
      [
        Shift.create(employee_id: employee.id, service_id: service.id, week_id: weeks[0].id, start_time: Time.now,
                     end_time: (Time.now + 1.hour), day: 1),
        Shift.create(employee_id: employee.id, service_id: service.id, week_id: weeks[1].id,
                     start_time: (Time.now + 1.week), end_time: (Time.now + 1.hour), day: 1),
        Shift.create(employee_id: employee.id, service_id: service.id, week_id: weeks[2].id, start_time: (Time.now + 2),
                     end_time: (Time.now + 1.hour), day: 1)
      ]
    end
  ...
  context 'before creation' do
    ...
    let!(:shift) do
      Shift.create(service_id: services[0].id, employee_id: employees[0].id, week_id: week.id,
                   start_time: Time.now.beginning_of_hour, end_time: (Time.now.beginning_of_hour + 1.hour), day: 1)
    end
    it 'cannot have an employee with two shifts at the same time' do
      expect do
        Shift.create(service_id: services[1].id, employee_id: employees[0].id, week_id: week.id,
                     start_time: Time.now.beginning_of_hour, end_time: (Time.now.beginning_of_hour + 1.hour), day: 1)
      end.to raise_error(ActiveRecord::RecordNotUnique)
    end
    it 'cannot have a service with two shifts at the same time' do
      expect do
        Shift.create(employee_id: employees[1].id, service_id: services[0].id, week_id: week.id,
                     start_time: Time.now.beginning_of_hour, end_time: (Time.now.beginning_of_hour + 1.hour), day: 1)
      end.to raise_error(ActiveRecord::RecordNotUnique)
    end
